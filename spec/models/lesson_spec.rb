...
describe Lesson do
  ...
  describe 'new lesson' do
    ...
    describe 'after adding first new video' do
      before do
        @video1 = @new_lesson.videos.create(
          name: 'video',
  ...
  describe '#watched?' do
    context 'when user has watched all videos from lesson' do
      before do
        @lesson = create(:lesson)
        video1 = create(:video, lesson: @lesson)
        video2 = create(:video, lesson: @lesson)

        @user = create(:user)
        profile = create(:profile, user: @user)
        create(:profile_video, profile: profile, video: video1)
        create(:profile_video, profile: profile, video: video2)
      end

      it 'should be true' do
        expect(@lesson.watched?(@user)).to be true
      end
    end

    context 'when user has not watched all videos from lesson' do
      before do
        @lesson = create(:lesson)
        video1 = create(:video, lesson: @lesson)
        video2 = create(:video, lesson: @lesson)
      ...
      it 'should be false' do
        expect(@lesson.watched?(@user)).to be false
      end
    end

    context 'when user has not watched any videos from lesson' do
      before do
        @lesson = create(:lesson)
        video1 = create(:video, lesson: @lesson)
        video2 = create(:video, lesson: @lesson)
