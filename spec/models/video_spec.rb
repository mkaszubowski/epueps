...
RSpec.describe Video, type: :model do
  ...
  describe '#watched?' do
    context 'when user has watched video' do
      before do
        @user = create(:user)
        profile = create(:profile, user: @user)
        profile_video = create(:profile_video, profile: profile)
        @video = profile_video.video
      end

      it 'should be true' do
        expect(@video.watched?(@user)).to eq true
      end
    end

    context 'when user has not watched video' do
      before do
        @user = create(:user)
        @video = create(:video)
      end
